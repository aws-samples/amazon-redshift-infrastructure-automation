...
def handler(event, context):
    # data_request_type = event['data_request_type'].lower()
    # in_schema = event['in_schema']
    # data_request_size = int(event['data_request_size'])
    data_request_type = os.environ['data_request_type']
    in_schema = os.environ['in_schema']
    data_request_size = int(os.environ['data_request_size'])
    try:
        start = time.time()
        if data_request_type == 'iot' and in_schema == 'N':
            f_schema_less(data_request_size, data_request_type)
        elif data_request_type.lower() == 'finance' and in_schema == 'N':
            f_schema_less(data_request_size, data_request_type)
        elif data_request_type.lower() == 'hr' and in_schema == 'N':
            f_schema_less(data_request_size, data_request_type)
        elif data_request_type.lower() == 'hr-personal' and in_schema == 'N':
            f_schema_less(data_request_size, data_request_type)
        elif data_request_type.lower() == 'banking' and in_schema == 'N':
            f_schema_less(data_request_size, data_request_type)
        elif data_request_type.lower() == 'automotive' and in_schema == 'N':
            f_schema_less(data_request_size, data_request_type)
        elif data_request_type.lower() == 'sales' and in_schema == 'N':
            f_schema_less(data_request_size, data_request_type)
        elif in_schema == 'Y':
            dfs1 = f_schema(data_request_size)
            print(dfs1)
        end = time.time()
        print('Time taken to generate ' + data_request_type + ' is ' + str(end - start) + ' seconds')
